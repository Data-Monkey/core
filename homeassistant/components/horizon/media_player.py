...
class HorizonDevice(MediaPlayerEntity):
    ...
    _attr_supported_features = SUPPORT_HORIZON
    ...
    def _send(self, key=None, channel=None):
        ...
        try:
            ...
            _LOGGER.error("%s disconnected: %s. Trying to reconnect", self.name, msg)
            ...
            try:
                ...
                _LOGGER.error("Authentication to %s failed: %s", self.name, msg)
                return
            except OSError as msg:
                # occurs if horizon box is offline
                _LOGGER.error("Reconnect to %s failed: %s", self.name, msg)
                return
